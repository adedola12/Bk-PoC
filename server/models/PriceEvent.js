...
/** D10 append-only price ledger. Prices are NEVER overwritten â€” only appended. */
...
priceEventSchema.pre(["updateOne", "findOneAndUpdate", "updateMany"], function (next) {
  next(new Error("price_events is append-only (D10) â€” insert a new event instead"));
});

export const PriceEvent = mongoose.model("PriceEvent", priceEventSchema);
